...
export default {
  ...
  mounted(){
    ...
    foot.style.paddingBottom = Number(getname('safeAreaBottom')) + 'px'
  },
  methods:{
    getProMsg() {
    ...
    buyNow() {
      ...
      if (checkLoginStatus()) {
        ...
        // console.log(that.)
        https({
          url: siteUrl +'Main/Shopping/CheckProductBuyStatus',
          method: 'get',
          data:{
            uid: window.api ? api.getPrefs({sync: true,key: 'SessionUserID'}) : $api.getStorage('SessionUserID'),
            pid:that.$props.pid,
