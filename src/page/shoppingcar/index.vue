<template>
  <div
    v-show="dataover"
    id="app2"
    :style="cardata.length==0&&vipcardata.length==0?'':'margin-bottom:50px'"
  >
    <div
      class="catoubu"
      :style="statustop?'height:'+sheight+'px;padding-top:'+statustop+'px':''"
    >购物车</div>
    ...
    <section v-if="cardata.length>0" style="background:#fff">
      <div class="blockone">
        <div
          class="select"
          :style="zongcarselect[0]?'':'border:1px solid #D8D8D8'"
          @click="allcardata(0,0)"
        >
      ...
      <div class="prolist" v-for="(item,index) in cardata" :key="index">
        ...
        <div class="shopnamebox">
          <div class="items">
            <div
              class="select"
              :style="ptshopSelect[index]?'':'border:1px solid #D8D8D8'"
              @click="ShopAllSelect(index,0)"
            >
        ...
        <div v-for="(list,idx) in item" :key="list.shopid" style="position: relative;">
          <div
            class="pro-main"
            :style="putongselctin[index][idx].delelshow?'transform:translateX(-80px)':''"
          >
            <div
              class="select"
              :style="putongselctin[index][idx].onselect|onselect"
              @click="changselect(index,idx,putongselctin[index][idx].onselect,0)"
            >
              <img
                src="../../img/shoppingcar-select.png"
                alt
                v-show="putongselctin[index][idx].onselect==1"
              >
            </div>
            ...
            <div
              class="pro-xiangqing"
              @touchstart="touchstartF"
              @touchend="touchmoveF($event,index,idx,0)"
            >
              <div class="pro-title" @click="gopro(list.proid)">
                <p>
                  <span class="allquer" v-if="list.iscross">全球购</span>
                  <span class="xianshi" v-if="list.isrushbuy">限时抢购</span>
                  {{list.productname}}
                </p>
              </div>
              ...
              <div class="pro-jiage">
                ...
                <div class="numbox">
                  <i
                    class="iconfont icon-subtract_icon"
                    :style="list.minbuy|minbuyshow(putongselctin[index][idx].qty)"
                    @click="changenum(false,list.minbuy,putongselctin[index][idx].qty,list.id,index,idx,0)"
                  ></i>
                  <span
                    class="num"
                    @click="ChangeCount(list.minbuy,list.maxbuy,putongselctin[index][idx].qty,list.id,index,idx,0)"
                  >{{putongselctin[index][idx].qty}}</span>
                  <i
                    class="iconfont icon-plus_icon"
                    @click="changenum(true,list.maxbuy,putongselctin[index][idx].qty,list.id,index,idx,0)"
                  ></i>
                </div>
              </div>
            </div>
          ...
          <div
            class="pro-delel"
            @click="delpro(list.id)"
            :style="putongselctin[index][idx].delelshow?'z-index:0':''"
          >删除</div>
        </div>
      </div>
    </section>
    ...
    <section v-if="vipcardata.length>0">
      <div class="blockone">
        <div
          class="select"
          :style="zongcarselect[1]?'':'border:1px solid #D8D8D8'"
          @click="allcardata(1,1)"
        >
      ...
      <div class="prolist" v-for="(item,index) in vipcardata" :key="index">
        ...
        <div class="shopnamebox">
          <div class="items">
            <div
              class="select"
              :style="vipshopSelect[index]?'':'border:1px solid #D8D8D8'"
              @click="ShopAllSelect(index,1)"
            >
        ...
        <div v-for="(list,idx) in item" :key="list.shopid" style="position: relative;">
          <div
            class="pro-main"
            :style="vipselctin[index][idx].delelshow?'transform:translateX(-80px)':''"
          >
            <div
              class="select"
              :style="vipselctin[index][idx].onselect|onselect"
              @click="changselect(index,idx,vipselctin[index][idx].onselect,1)"
            >
              <img
                src="../../img/shoppingcar-select.png"
                alt
                v-show="vipselctin[index][idx].onselect==1"
              >
            </div>
            <div style="position:relative">
              <img class="pro_img" :src="list.pic" alt @click="gopro(list.proid)">
            ...
            <div
              class="pro-xiangqing"
              @touchstart="touchstartF"
              @touchend="touchmoveF($event,index,idx,1)"
            >
              <div class="pro-title" @click="gopro(list.proid)">
                <p>
                  <span class="allquer" v-if="list.iscross">全球购</span>
              ...
              <div class="pro-jiage">
                ...
                <div class="numbox">
                  <i
                    class="iconfont icon-subtract_icon"
                    :style="list.minbuy|minbuyshow(vipselctin[index][idx].qty)"
                    @click="changenum(false,list.minbuy,vipselctin[index][idx].qty,list.id,index,idx,1)"
                  ></i>
                  <span
                    class="num"
                    @click="ChangeCount(list.minbuy,list.maxbuy,vipselctin[index][idx].qty,list.id,index,idx,1)"
                  >{{vipselctin[index][idx].qty}}</span>
                  <i
                    class="iconfont icon-plus_icon"
                    @click="changenum(true,list.maxbuy,vipselctin[index][idx].qty,list.id,index,idx,1)"
                  ></i>
                </div>
              </div>
            </div>
          ...
          <div
            class="pro-delel"
            @click="delpro(list.id)"
            :style="vipselctin[index][idx].delelshow?'z-index:0':''"
          >删除</div>
        </div>
      </div>
    </section>
    ...
    <section v-if="cardata.length>0||vipcardata.length>0">
      ...
      <div class="btnbox">
        <p class="setprioc">
          总计：
          <em style="color:#F5AC9E">&yen;</em>
          <span>{{allPrice|toFixed}}</span>
        </p>
    ...
    <coupon
      ...
      :bindData="showCoupon"
      way="shoppingcar"
      @closeCou="showCoupon=!showCoupon"
      :cid="cid"
    ></coupon>
...
export default {
  ...
  filters: {
    ...
    minbuyshow: function(value, qty) {
      if (value != 0) {
        //则设置了最小购买量
        if (qty == value) return "color:#999999";
      } else {
        if (qty == 1) {
          return "color:#999999";
        }
  ...
  methods: {
              ...
              ret.Data.json.forEach(function(item, index) {
                ...
                item.forEach(function(list, li) {
                  ...
                  if (list.rushbuystatus == 1 || list.rushbuystatus == 2) {
                    ...
                    list.rushbuystatus == 0 &&
                    list.prorushbuy &&
                    list.isrushbuy
                  ) {
                    console.log(
                      list.prorushbuy.begintimeticks < list.nowtimetick
                    );
    ...
    touchmoveF(event, index, idx, where) {
      ...
      if (
        that.swipeX &&
        Math.abs(that.X - that.x) - Math.abs(that.Y - that.y) > 0
      ) {
        // 阻止事件冒泡
