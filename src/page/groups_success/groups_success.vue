      ...
      <div>
        <div class="gpbox">
          <div v-if="msgdata.count">
            <div class="gp1" style="color:#9B9B9B;">
              还差
              <span class="numone">{{msgdata.count}}人</span>成团
            </div>
            <div class="gp2">
              <span
                style="color:#9B9B9B;"
              >距离结束还剩下 {{days<=0?'':day+'天'}}{{hours}}:{{minutes}}:{{seconds}}</span>
            </div>
          </div>
          <div v-else>
          ...
          <div class="items xiabox" :style="allping.length>=4?'width:80%;flex-wrap:wrap':''">
            ...
            <div class="canyu" v-for="(item,index) in allping" :key="index">
              <img
                v-if="msgdata.resobj[index+1]"
                :src="msgdata.resobj[index+1].userimg?msgdata.resobj[index+1].userimg:(!msgdata.resobj[index+1].userimg?'./img/man.jpg':'./img/man.jpg')"
              >
              <img v-else src="../../img/hui.png" alt>
              <div
                v-if="msgdata.resobj[index+1]"
                class="or_name"
              >{{msgdata.resobj[index+1].username|isnumber}}</div>
...
export default {
  ...
  filters: {
    isnumber: function(value) {
      ...
      if (!isNaN(value) && value.length == 11) {
        var tel = value.replace(value.slice(-8, -3), "****");
        return tel;
      } else {
        return value;
      }
  ...
  methods: {
    getMsg() {
      var that = this;
      https({
        url: siteUrl + "/Marketing/Groupbuy/GetMyGroupbuyDetails?devicetype=1",
        data: {
        ...
        successBack: function(ret) {
          ...
          if (ret.status == 1 && ret.success) {
            ...
            let allping = [];
            var numm =
              ret.Data[0].resobj.length > ret.Data[0].gbcount
                ? ret.Data[0].resobj.length
                : ret.Data[0].gbcount;
            ...
            (that.msgdata = ret.Data[0]), (that.allping = allping);
            that.okone = true;
          } else {
            promptMsg(ret.err);
          }
...
.commoneheaderleft img,
.commoneheaderright img {
  height: 20px;
  width: 20px;
  display: block;
}
