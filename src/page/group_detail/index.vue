<template>
  <div>
      ...
      <div class="g_main">
              ...
              <div class="pro_xian">
                <div class="p1">
                  <span
                    ...
                    :style="msgdata.buyonegetone || msgdata.headfree ? '' : 'padding: 0'"
                    class="allhave"
                  >
                    <span
                      :style="msgdata.buyonegetone || msgdata.headfree ? 'margin-right: 5px;' : 'margin: 0;'"
                      class="allhaveping"
                    >{{msgdata.gbnum}}人团</span>
                    {{msgdata.headfree
                    ? '团长免单 ' : ''}}{{msgdata.buyonegetone ? '拼一送一' : ''}}
                  </span>
                  {{msgdata.proname}}
                </div>
              </div>
        ...
        <div class="gpbox">
          ...
          <div class="gp2" v-if="msgdata.gbstate==1&&!overtime">
            <span>
              还差
              <span style="color:#FF5541">{{msgdata.num}}</span>人 距离结束
            </span>
            <span
              style="color:#FF5541;margin-left:8px;"
            >{{days<=0?'':day+'天'}}{{hours}}:{{minutes}}:{{seconds}}</span>
          </div>
          <div class="gp2" v-if="msgdata.gbstate>=3">
            <span>本次未成团，退款已原路退回</span>
          </div>
          <div class="items xiabox" :style="allping.length>=3?'width:80%;flex-wrap:wrap':''">
            <div class="canyu">
              <img v-if="msgdata.imgphoto" :src="msgdata.imgphoto" @error="onerr($event)">
            ...
            <div class="canyu" v-for="(item,index) in allping" :key="index">
              <img
                v-if="msgdata.groupbuylist[index]"
                :src="msgdata.groupbuylist[index].head?msgdata.groupbuylist[index].head:(msgdata.groupbuylist[index].head===''?'./img/man.jpg':'./img/hui.png')"
              >
              <img v-else src="../../img/hui.png">
              <div
                class="or_name"
                v-if="msgdata.groupbuylist[index]"
              >{{msgdata.groupbuylist[index].name|isnumber}}</div>
    ...
    <div class="btnzu">
      ...
      <div class="wxbtn appbtn yanbtn" v-else @click="gobuy">{{btnmessg}}</div>
    </div>
  </div>
</template>
...
export default {
  ...
  methods: {
    bindata: function() {
      ...
      that.onec = true;
      https({
        url: siteUrl + "/Marketing/Groupbuy/GetGBOrder?devicetype=1",
        data: {
        ...
        successBack: ret => {
          if (ret.status == 1 && ret.success) {
            clearInterval(that.tiomeout_ores);
            let remaintime = "";
            let commit = "";
            var agroudtimes = "";
            var btnmessg = "";
            ...
            if (ret.Data.gbstate == 1) {
              ...
              if (remaintime > 0) {
                ...
                that.tiomeout_ores = setInterval(agroudtimes, 1000);
                //后台有延迟 直接前端
              } else {
                clearInterval(that.tiomeout_ores);
                //有这个参数代表和没参加这个拼团
            ...
            //有几个人需要参团
            let allping = [];
            var numm = ret.Data.count > ret.Data.gbnum ? ret.Data.count : ret.Data.gbnum
